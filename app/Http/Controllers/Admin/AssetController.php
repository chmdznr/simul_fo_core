...
class AssetController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Asset::with(['category', 'status', 'assigned_to', 'site'])->select(sprintf('%s.*', (new Asset())->table));
            ...
            $table->editColumn('aid', function ($row) {
                return $row->aid ? $row->aid : '';
            });
            ...
            $table->addColumn('site_sid', function ($row) {
                return $row->site ? $row->site->sid : '';
            });

            $table->editColumn('site.site_name', function ($row) {
                return $row->site ? (is_string($row->site) ? $row->site : $row->site->site_name) : '';
            });

            $table->rawColumns(['actions', 'placeholder', 'category', 'photos', 'status', 'assigned_to', 'site']);
        ...
        return view('admin.assets.index', compact('asset_categories', 'asset_statuses', 'users', 'site_points'));
    }
    ...
    public function create()
    {
        ...
        $sites = SitePoint::pluck('sid', 'id')->prepend(trans('global.pleaseSelect'), '');

        return view('admin.assets.create', compact('assigned_tos', 'categories', 'sites', 'statuses'));
    }
    ...
    public function edit(Asset $asset)
    {
        ...
        $asset->load('category', 'status', 'assigned_to', 'site');

        return view('admin.assets.edit', compact('asset', 'assigned_tos', 'categories', 'sites', 'statuses'));
    }
    ...
    public function show(Asset $asset)
    {
        ...
        $asset->load('category', 'status', 'assigned_to', 'site', 'assetConnectorCoreConfigs');
