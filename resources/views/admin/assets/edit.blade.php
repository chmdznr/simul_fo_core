...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.update", [$asset->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="aid">{{ trans('cruds.asset.fields.aid') }}</label>
                <input class="form-control {{ $errors->has('aid') ? 'is-invalid' : '' }}" type="text" name="aid" id="aid" value="{{ old('aid', $asset->aid) }}" required>
                @if($errors->has('aid'))
                    <div class="invalid-feedback">
                        {{ $errors->first('aid') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.aid_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="brand">{{ trans('cruds.asset.fields.brand') }}</label>
                <input class="form-control {{ $errors->has('brand') ? 'is-invalid' : '' }}" type="text" name="brand" id="brand" value="{{ old('brand', $asset->brand) }}">
                @if($errors->has('brand'))
                    <div class="invalid-feedback">
                        {{ $errors->first('brand') }}
                    </div>
            ...
            <div class="form-group">
                <label for="site_id">{{ trans('cruds.asset.fields.site') }}</label>
                <select class="form-control select2 {{ $errors->has('site') ? 'is-invalid' : '' }}" name="site_id" id="site_id">
                    @foreach($sites as $id => $entry)
                        <option value="{{ $id }}" {{ (old('site_id') ? old('site_id') : $asset->site->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('site'))
                    <div class="invalid-feedback">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.asset.fields.site_helper') }}</span>
            </div>
            <div class="form-group">
