...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.segmentConnected.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.segment-connecteds.update", [$segmentConnected->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="segid">{{ trans('cruds.segmentConnected.fields.segid') }}</label>
                <input class="form-control {{ $errors->has('segid') ? 'is-invalid' : '' }}" type="text" name="segid" id="segid" value="{{ old('segid', $segmentConnected->segid) }}" required>
                @if($errors->has('segid'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('alias') ? 'is-invalid' : '' }}" type="text" name="alias" id="alias" value="{{ old('alias', $segmentConnected->alias) }}">
                @if($errors->has('alias'))
                    <div class="invalid-feedback">
                        {{ $errors->first('alias') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('site_point_1') ? 'is-invalid' : '' }}" name="site_point_1_id" id="site_point_1_id" required>
                    @foreach($site_point_1s as $id => $entry)
                        <option value="{{ $id }}" {{ (old('site_point_1_id') ? old('site_point_1_id') : $segmentConnected->site_point_1->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('site_point_2') ? 'is-invalid' : '' }}" name="site_point_2_id" id="site_point_2_id">
                    @foreach($site_point_2s as $id => $entry)
                        <option value="{{ $id }}" {{ (old('site_point_2_id') ? old('site_point_2_id') : $segmentConnected->site_point_2->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_core') ? 'is-invalid' : '' }}" type="number" name="total_core" id="total_core" value="{{ old('total_core', $segmentConnected->total_core) }}" step="1" required>
