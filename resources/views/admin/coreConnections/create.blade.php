...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.coreConnection.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.core-connections.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('core_config') ? 'is-invalid' : '' }}" name="core_config_id" id="core_config_id" required>
                    @foreach($core_configs as $id => $entry)
                        <option value="{{ $id }}" {{ old('core_config_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('core_number_1') ? 'is-invalid' : '' }}" type="text" name="core_number_1" id="core_number_1" value="{{ old('core_number_1', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('core_number_2') ? 'is-invalid' : '' }}" type="text" name="core_number_2" id="core_number_2" value="{{ old('core_number_2', '') }}" required>
